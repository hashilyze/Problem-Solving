...
// === File Control ====
const std::string FILE_NAME = "mice";
...
int main(void){
    ...
    while(t--){
        in >> n >> k >> h >> m;
        for(int i = 0; i < n; ++i) in >> room[i].x >> room[i].y;
        for(int i = 0; i < h; ++i) in >> holes[i].x >> holes[i].y;
        for(int i = 0; i < m; ++i) in >> mouses[i].x >> mouses[i].y;
        initGraph();
        out << (maxflow(0, m + h + 1) == m ? "Possible\n" : "Impossible\n");
    }
#ifndef ONLINE_JUDGE
    fin.close();
    fout.close();
